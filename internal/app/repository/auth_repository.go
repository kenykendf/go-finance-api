...
func (ar *AuthRepo) GetByID(userID int, refreshToken string) (model.Auth, error) {
	var (
		sqlStatement = `
...
func (ar *AuthRepo) CreateSession(auth model.Auth) error {
	var (
		sqlStatement = `
...
func (ar *AuthRepo) DeleteByUserID(userID int) error {
	var (
		sqlStatement = `
			DELETE FROM auth
			WHERE user_id = $1
